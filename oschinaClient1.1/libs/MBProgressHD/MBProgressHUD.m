...
//    [self cleanUp];
//    [[NSNotificationCenter defaultCenter]postNotificationName:NOTIFICATION_TYPE_XML_STOPNETWORKREQUEST object:nil userInfo:nil];
//}
...
- (void)layoutSubviews {
	...
	labelSize.width = MIN(labelSize.width + kPadding *2, maxWidth);
	totalSize.width = MAX(totalSize.width, labelSize.width);
	totalSize.height += labelSize.height;
	if (labelSize.height > 0.f && indicatorF.size.height > 0.f) {
		totalSize.height += kPadding *1;
	}else{
        totalSize.height += kPadding *2;
    }
	...
	// Position elements
	CGFloat yPos = roundf(((bounds.size.height - totalSize.height) / 2)) + margin/2 + yOffset;
